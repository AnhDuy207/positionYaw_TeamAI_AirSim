...
class OffboardControl {
 ...
 private:
	...
	//DuyNguyen
	ros::Subscriber pub_Opt_point_sub_;
	...
	//DuyNguyen
	geometry_msgs::Point pub_Opt_point_;

	// point (x,y,z) list received from optimization planner, use when want to buffer and check reached each optimization point
	std::vector<geometry_msgs::Point> optimization_point_;
	// current GPS informations from mavros: status (satellite fix status information):
	...
	//DuyNguyen
	bool pub_point_received_ = false;
	...
	// the offset to check when the drone reached the setpoints (for ENU, GPS and land, corresponding)
	double target_error_, goal_error_, land_error_;
	double distance_;  // distance from current position to next setpoint
	
	// array to calculate offset from current ENU (x,y,z) and GPS converted (x,y,z) in a period
	...
	// the height (set to 0.0 for land to ground - need to set disable auto-disarm of pixhawk)
	// want drone go to for delivery in delivery mode
	double z_delivery_;
	...
	// corresponding hover time when reached setpoint, when takeoff and when unpacking
	double hover_time_, takeoff_hover_time_, unpack_time_;
	...
	//DuyNguyen
	void pubOptPointCallback(const geometry_msgs::Point::ConstPtr& msg);
	...
	template <class T>  // calculate square of x
    inline T sqr(T x) {
        return x*x;
    }; 
	...
	//DuyNguyen
	void inputPublishPoints();
	void publishPointsFlight();
	...
	geometry_msgs::Vector3 velComponentsCalc(double v_desired,
