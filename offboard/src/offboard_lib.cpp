  ...
  return_home_mode_enable_(false)
{
    ...
    // opt_point_sub_ = nh_.subscribe("optimization_point", 10, &OffboardControl::optPointCallback, this);    //planning
    pub_Opt_point_sub_ = nh_.subscribe("optimization_point", 10, &OffboardControl::pubOptPointCallback, this);
...
void OffboardControl::pubOptPointCallback(const geometry_msgs::Point::ConstPtr& msg) {
    pub_Opt_point_ = *msg;
    //optimization_point_.push_back(*msg);
    pub_point_received_ = true;
}
...
void OffboardControl::inputSetpoint() {
    ...
    if (mode == '1') {
        ...
        IVSR_LOGI("- Choose (4): Fly with Point pub");
        std::printf("(1/2/3/4): ");
        std::cin >> c;
        if (c == '1') {
...
//DuyNguyen
/* manage for flight with optimization point from planner */
void OffboardControl::inputPublishPoints() {
    ros::Rate rate(30.0);  //default: 10.0
    waitForStable(30.0); //default: 10.0
    setOffboardStream(30.0,targetTransfer(current_odom_.pose.pose.position.x, current_odom_.pose.pose.position.y, z_takeoff_)); //default: 10.0
    waitForArmAndOffboard(10.0); //default: 10.0
    while (!pub_point_received_)
    {
        takeOff(targetTransfer(current_odom_.pose.pose.position.x, current_odom_.pose.pose.position.y, z_takeoff_), 1); //takeoff_hover_time_
    }
    IVSR_LOGI("Flight with publish setpoint");
    IVSR_LOGI("Flighting to start point of Optimization path");
    publishPointsFlight();
}

void OffboardControl::publishPointsFlight() {
    bool final_reached = false;
    geometry_msgs::PoseStamped setpoint;
    ros::Rate rate(30.0);
    if (pub_point_received_) {
        IVSR_LOGI("Received publish points!");
        double target_alpha, this_loop_alpha;
        while (ros::ok()) {
            setpoint = targetTransfer(pub_Opt_point_.x, pub_Opt_point_.y, pub_Opt_point_.z);
            
            target_alpha = calculateYawOffset(targetTransfer(current_odom_.pose.pose.position.x,
                                                            current_odom_.pose.pose.position.y,
                                                            current_odom_.pose.pose.position.z),
                                            targetTransfer(pub_Opt_point_.x, pub_Opt_point_.y, pub_Opt_point_.z));
            ...
            target_enu_pose_.pose.position.x = pub_Opt_point_.x; 
            target_enu_pose_.pose.position.y = pub_Opt_point_.y; 
            target_enu_pose_.pose.position.z = pub_Opt_point_.z; 
